...
function vtlo_save_settings_func(){
  ...
  foreach($_POST as $k=>$v){
    if($k!='action' && $k!='_ajax_nonce' && $v!='no_save'){
      if(isset($_POST['keephtml_'.$k])){
        $value=$v;
      }else{
...
function get_vtlo_select($id,$options){
  ...
  foreach($options as $key=>$val){
    $selected='';
    if(get_option($id,'')==$key){$selected='selected';}
...
function get_vtlo_multiimgupload($id){
    ...
    foreach($getimgs as $i){
      $img_src = wp_get_attachment_image_src($i,'thumbnail');
      echo '<span id="multiimg_'.$i.'_'.$id.'" class="'.$id.'_multiimg multiimg"><img src="'.$img_src[0].'" style="width:60px;height:60px;" alt="" /><br><a style="color:#B8302E;" onclick="remove_multiimg(\''.$id.'\',\''.$i.'\');">'.__('remove','vtlo').'</a></span>';
    }
...
function get_vtlo_html_textarea($id){
  global $settings_list,$keep_html;
  $settings_list[]=$id;
  $the_tags='';
  foreach($keep_html as $key=>$v){$display_key[]=$key;};
  $html='<input type="hidden" name="keephtml_'.$id.'" value="no_save"><textarea name="'.$id.'" id="'.$id.'" cols="30" rows="10" style="width:100%">'.stripslashes(get_option($id,'')).'</textarea>';
  return $html;
}
...
function get_vtlo_input($id){
    global $settings_list;
  $settings_list[]=$id;
  $html='<input type="text" name="'.$id.'" id="'.$id.'" value="'.htmlspecialchars(stripslashes(get_option($id,''))).'" />';
  return $html;
}
